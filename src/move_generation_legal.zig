...
fn getCheckers(color: board.Color, stateBoards: board.StateBoards, king: u64) u64 {
    ...
    // If WHITE: there should be black pawns above to the left and right of the king
    // If Black: there should be white pawns below to the left and right of the king
	const direction = if (color == board.Color.White)  -1  else  1 ;
...
test "GetCheckers" {
    ...
    const bb = getCheckers(b.Color, b.stateBoards);
...
pub fn LegalMovesWhite(b: *board.Board, moveList: *board.MoveList) void {
	_ = moveList;
	_ = b;
}
...
pub fn LegalMovesBlack(b: *board.Board, moveList: *board.MoveList) void {
	_ = moveList;
	_ = b;
}
// 	board.UpdateBitMasks()
